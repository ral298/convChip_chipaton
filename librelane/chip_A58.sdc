...
# ------------------------------------------------------------------------------
# Delays de Entrada y Salida
# ------------------------------------------------------------------------------
set_input_delay -min 0 -clock $clocks $input_ports
set_input_delay -max $input_delay_value -clock $clocks $input_ports

set_output_delay $output_delay_value -clock $clocks $output_ports

# ------------------------------------------------------------------------------
# Exclusión de Señales Estáticas de Configuración
# ------------------------------------------------------------------------------
set_false_path -to [get_ports { \
    *_PU *_PD *_OE *_IE *_CS *_SL \
}]
...
if { [llength [get_ports -quiet input_PAD[*]]] > 0 } {
    set_input_delay -min 0 -clock $clocks [get_ports input_PAD[*]]
    set_input_delay -max $input_delay_value -clock $clocks [get_ports input_PAD[*]]
}

# Retardo de entrada para rst_n_PAD
set_input_delay -min 0 -clock $clocks [get_ports rst]
set_input_delay -max 2.0 -clock $clocks [get_ports rst]
...
# Forzar transición máxima estricta en el puerto de reset de entrada
set_input_transition 0.5 [get_ports rst]

# Declarar la línea de reset como False Path para evitar falsos negativos en Setup/Hold
#set_false_path -from [get_ports rst]
#set_false_path -to [get_pins i_chip_core.con32_one_instruction_u/rst]
