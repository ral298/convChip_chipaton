...
module uart_ultimo #(
// Setup parameters
parameter CLK_IN_HZ    =  50000000,//42000000,    // Set to system input clock frequency
parameter BAUD_RATE    =   3000000       // Set to desired baud rate
)(
	...
	begin
		...
		if ( rx_trigger )	begin                                        // if a 'rx_trigger' event has taken place
			...
			//begin
				...
				else begin                                 // if the receiver period counter has not reached it's end
		...
		if (ena_tx & (~tx_busy)) begin                    // If a transmit request comes in
			tx_data_reg    <= tx_data ;  // register a copy of the input data bus
			tx_busy        <= 1'b1 ;        // Set the busy flag
			//ena_tx_busy		<=1;
		end
		...
		tx_last_busy <= tx_run; 
		tx_rdy       <= tx_last_busy && ~tx_run;
		*/
		
		//He agregado este if, el cual representa que cuando tx_rdy es 1 significa que ya podemos mandar un dato para NO perder tiempo de un paquete de 8 bits
		if ( tx_position==5 & tx_run & tx_period==(TX_PERIOD/2))
			tx_rdy=1'b1;
		else
			tx_rdy=1'b0;
		...
		if (  rx_trigger && ~tx_run ) begin
				...
				tx_period      <= (TX_PERIOD[15:0])+3'b010;//los bits sumando es para sincronizar completamente la señal, dandole asi un offset en el tiempo sumando 3 ciclos de reloj.
											// reset "SYNCHRONIZE" the transmit period timer to the rx_trigger event, recognizing that the rx_trigger is
